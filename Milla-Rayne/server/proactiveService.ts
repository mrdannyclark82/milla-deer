  ...
  try {
    ...
    try {
      ...
      const timeSinceLastInteraction =
        Date.now() - existingActivity.lastInteraction;
      ...
      activity = {
        ...
        continuousActivityStart: wasOnBreak
          ? Date.now()
          : existingActivity.continuousActivityStart || Date.now(),
        lastBreakTaken: wasOnBreak
          ? existingActivity.lastInteraction
          : existingActivity.lastBreakTaken,
      };
    } catch {
    ...
    activityUpdateTimeout = setTimeout(async () => {
      if (pendingActivityUpdate) {
        try {
          await fs.writeFile(
            ACTIVITY_FILE,
            JSON.stringify(pendingActivityUpdate, null, 2)
          );
...
export async function checkMilestones(): Promise<string | null> {
  try {
    const memoryContext = await getMemoryCoreContext(
      'relationship milestones memories together'
    );
    ...
    if (
      memoryContext.includes('first') ||
      memoryContext.includes('anniversary')
    ) {
...
export function detectEnvironmentalContext(): string {
  ...
  if (month >= 2 && month <= 4) contexts.push('spring brings renewed energy');
    ...
    contexts.push('summer warmth encourages connection');
  else if (month >= 8 && month <= 10)
    contexts.push("autumn's beauty inspires reflection");
...
export async function generateBreakReminder(): Promise<string | null> {
  try {
    ...
    const now = Date.now();
    const continuousActivityTime =
      now - (activity.continuousActivityStart || now);
    ...
    const shouldRemindForBreak =
      (continuousActivityTime > 90 * 60 * 1000 &&
        continuousActivityTime < 95 * 60 * 1000 &&
        timeSinceLastReminder > 25 * 60 * 1000) || // 30 min warning
      (continuousActivityTime > 110 * 60 * 1000 &&
        continuousActivityTime < 115 * 60 * 1000 &&
        timeSinceLastReminder > 15 * 60 * 1000) || // 10 min warning
      (continuousActivityTime > 120 * 60 * 1000 &&
        timeSinceLastReminder > 10 * 60 * 1000); // Break time (2 hours)
    ...
    if (workMinutes >= 120) {
      if (hour >= 6 && hour <= 11) {
        breakMessages.push(
          `Perfect morning break - step outside for fresh air, grab a healthy snack, or just stretch in the sunlight.`
        );
      } else if (hour >= 12 && hour <= 14) {
        breakMessages.push(
          `Great time for a lunch break! Have you eaten yet? A proper meal and some movement will recharge you perfectly.`
        );
      } else if (hour >= 15 && hour <= 17) {
        breakMessages.push(
          `Afternoon break time - beat that energy dip with some movement, hydration, or a quick walk outside.`
        );
      } else if (hour >= 18 && hour <= 22) {
        breakMessages.push(
          `Evening break - step away from screens, do some gentle stretches, or just relax and reset your mind.`
        );
      }
    }
...
export async function checkBreakReminders(): Promise<{
  ...
  try {
    ...
    if (hour >= 6 && hour <= 11) {
      welcomeMessages.push(
        `Good morning energy! Hope your break included some fresh air or a healthy snack. Ready to make this morning productive?`
      );
    } else if (hour >= 12 && hour <= 14) {
      welcomeMessages.push(
        `Back from lunch break? I hope you ate something nourishing. Afternoon energy is perfect for focused work!`
      );
    } else if (hour >= 15 && hour <= 17) {
      welcomeMessages.push(
        `Afternoon refresher break done! That mid-day reset is so important. Ready to power through the rest of the day?`
      );
    } else if (hour >= 18 && hour <= 22) {
      welcomeMessages.push(
        `Evening break complete! Hope you stepped away from screens for a bit. Ready for some more focused time together?`
      );
    }
...
export async function checkPostBreakReachout(): Promise<{
