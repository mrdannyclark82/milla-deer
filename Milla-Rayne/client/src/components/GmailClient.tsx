...
export function GmailClient() {
  const [emails, setEmails] = useState<Email[]>([
    {
      ...
      preview:
        'Dependabot found a new vulnerability in your package-lock.json...',
      date: '10:30 AM',
      isRead: false,
      isStarred: true,
    },
    {
      ...
      preview:
        'The latest deployment to production was successful. View details...',
      date: 'Yesterday',
      isRead: true,
      isStarred: false,
    },
  ...
  const [activeTab, setActiveTab] = useState<'inbox' | 'sent' | 'trash'>(
    'inbox'
  );
    ...
    <Card className="bg-black/40 backdrop-blur-lg border border-white/10 h-full flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-48 border-r border-white/10 p-2 space-y-1 hidden md:block">
          ...
          <Button
            variant="ghost"
            className="w-full justify-start text-sm text-white/60 hover:text-white hover:bg-white/5"
          >
            ...
            {emails.map((email) => (
              ...
              >
                <div className="flex items-start gap-3">
                  <div
                    className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${!email.isRead ? 'bg-blue-500' : 'bg-transparent'}`}
                  />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <span
                        className={`text-sm ${!email.isRead ? 'font-semibold text-white' : 'text-white/80'}`}
                      >
                        {email.from}
                      </span>
                    ...
                    <h4
                      className={`text-sm mb-1 ${!email.isRead ? 'font-medium text-white' : 'text-white/70'}`}
                    >
                      {email.subject}
                    </h4>
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8 text-white/20 hover:text-yellow-400 -mt-1"
                  >
                    <Star
